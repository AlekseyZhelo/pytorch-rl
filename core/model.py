...
class A3CMlpModel(Model):
    def __init__(self, args):
        ...
        # 1. policy output
        self.policy_5 = nn.Linear(self.hidden_vb_dim, self.output_dims)
        self.policy_6 = nn.Softmax()
        # 2. value output
        self.value_5 = nn.Linear(self.hidden_vb_dim, 1)
...
# currently best performing with two LSTM layers
class A3CMlpDeeperMinisimModel(Model):
    def __init__(self, args):
        super(A3CMlpDeeperMinisimModel, self).__init__(args)

        self.lstm_layer_count = 1  # workaround to keep the current behaviour with the hidden state
        self.num_robots = args.num_robots
        self.hist_len = args.hist_len
        self.hidden_vb_dim = args.hidden_vb_dim
        ...
        if self.enable_lstm:
            ...
            self.lstm2 = nn.LSTMCell(self.hidden_vb_dim, self.hidden_vb_dim, 1)
    ...
    def forward(self, x, lstm_hidden_vb=None):
        ...
        if self.enable_lstm:
            x, c = self.lstm(x, lstm_hidden_vb)
            x, c = self.lstm2(x, (x, c))  # TODO: have own (h, c) for the hidden and cell state?
        ...
        if self.enable_lstm:
            # TODO: should separate h and c for each LSTM layer?
            # would be difficult due to the coupling in a3cSingleProcess code
            return p, v, (x, c)
        else:
            return p, v
...
class A3CMlpDeeper2MinisimModel(Model):
    def __init__(self, args):
        super(A3CMlpDeeper2MinisimModel, self).__init__(args)

        self.lstm_layer_count = 2
        self.num_robots = args.num_robots
        self.hist_len = args.hist_len
        self.hidden_vb_dim = args.hidden_vb_dim
        # TODO: decided to hardcode for now
        self.hidden_vb2_dim = self.hidden_vb_dim + 2 * self.num_robots * self.hist_len
        ...
        if self.enable_lstm:
            ...
            final_input_size = self.hidden_dim // 4
        # 1. policy output
        self.policy_7 = nn.Linear(final_input_size, self.output_dims)
        self.policy_6 = nn.Softmax()
        # 2. value output
        self.value_8 = nn.Linear(final_input_size, 1)
    ...
    def forward(self, x, lstm_hidden_vb=None, lstm_hidden_vb2=None):
        ...
        if self.enable_lstm:
            x1, c1 = self.lstm(x, lstm_hidden_vb)
            x_aug = torch.cat((x1, target_data), 1)
            x2, c2 = self.lstm2(x_aug, lstm_hidden_vb2)
...
class A3CMlpNarrowingMinisimModel(Model):
    def __init__(self, args):
        ...
        if self.enable_lstm:
            ...
            final_input_size = self.hidden_dim // 4
        # 1. policy output
        self.policy_5 = nn.Linear(final_input_size + 2 * self.num_robots * self.hist_len, self.output_dims)
        self.policy_6 = nn.Softmax()
        # 2. value output
        self.value_5 = nn.Linear(final_input_size + 2 * self.num_robots * self.hist_len, 1)
