...
class A3CMlpMinisimModel(Model):
    def __init__(self, args):
        ...
        if self.enable_lstm:
            self.lstm = nn.LSTMCell(self.hidden_dim, self.hidden_dim, 1)
        # 1. policy output
        self.policy_5 = nn.Linear(self.hidden_dim + 2 * self.num_robots * self.hist_len, self.output_dims)
        self.policy_6 = nn.Softmax()
        # 2. value output
        self.value_5 = nn.Linear(self.hidden_dim + 2 *  self.num_robots * self.hist_len, 1)
...
class A3CMlpDeeperMinisimModel(Model):
    def __init__(self, args):
        ...
        if self.enable_lstm:
            ...
            self.lstm2 = nn.LSTMCell(self.hidden_dim // 4, self.hidden_dim // 4, 1)
        # 1. policy output
        self.policy_7 = nn.Linear(self.hidden_dim // 4 + 2 * self.num_robots * self.hist_len, self.output_dims)
        self.policy_6 = nn.Softmax()
        # 2. value output
        self.value_8 = nn.Linear(self.hidden_dim // 4 + 2 * self.num_robots * self.hist_len, 1)
